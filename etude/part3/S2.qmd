...
yearmap = tiled_basemap.opts(alpha=0.5) * \
         ...
         rasterize(overlay.select(annee=Annee), 
            aggregator=ds.reductions.count('annee')
                ...
                height=plot_height, 
                colorbar=True, 
                colorbar_opts={"ticker": FixedTicker(ticks=[2,4,6,8,10, 20])},
                color_levels=[0,2,4,6,8,10,20,20.1],
                clim=(0, 20),
                tools=[hover]
                )

viz_yearmap = (yearmap).opts(title=f"Répartition des surfaces brulées en {Annee} ", toolbar=None)
...
viz_allyearmap = (allyearmap).opts(title=f"Répartition des surfaces brulées de {int(annees[0])} à {int(annees[-1])}", toolbar=None)
...
saving = hv.save(viz_maps, f'{export_file_path}{figureName}.png', backend='bokeh',  title=f'{figureName}', fmt='png')
saving = hv.save(viz_maps, f'{export_file_path}{figureName}.html', backend='bokeh',  title=f'{figureName}', fmt='html')
# export des figures en images pour les afficher dans le rapport 
...
::: {#fig-map-annee-etude-S2}

```{python}
#| echo: false
#| label: fig-map-annee-etude-S2
...
::: {#fig-map-annee-par-annee-S2}

##### Répartitions des détéctions années après années
```{python}
#| echo: false
#| label: fig-map-annee-par-annee-S2
i=0
for maps in maps_batch:
    # export des figures en images par lot de 4 années pour les afficher dans le rapport 
